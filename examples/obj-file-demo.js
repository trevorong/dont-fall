...
export class AmongUs extends Shape {                                   // **Shape_From_File** is a versatile standalone Shape that imports
                                                                               // all its arrays' data from an .obj 3D model file.
    constructor(filename) {
        super("position", "normal", "texture_coord");
        // Begin downloading the mesh. Once that completes, return
    ...
    parse_into_mesh(data) {                           // Adapted from the "webgl-obj-loader.js" library found online:
            ...
            if (VERTEX_RE.test(line)) verts.push.apply(verts, elements);
                ...
                for (var j = 0, eleLen = elements.length; j < eleLen; j++) {
                    ...
                    if (elements[j] in unpacked.hashindices)
                        ...
                        if (textures.length) {
                            unpacked.textures.push(+textures[((vertex[1] - 1) || vertex[0]) * 3 + 0]);
                            unpacked.textures.push(+textures[((vertex[1] - 1) || vertex[0]) * 3 + 1]);
                            unpacked.textures.push(+textures[((vertex[1] - 1) || vertex[0]) * 3 + 2]);
                        }
        ...
        }
        {
            ...
            for (var j = 0; j < verts.length / 3; j++) {
                this.arrays.position.push(vec3(verts[3 * j], verts[3 * j + 1], verts[3 * j + 2]));
                this.arrays.normal.push(vec3(norms[3 * j], norms[3 * j + 1], norms[3 * j + 2]));
                this.arrays.texture_coord.push(vec(textures[3 * j], textures[3 * j + 1]/*, textures[3 * j + 2]*/));
            }
            this.indices = unpacked.indices;
        }
        this.normalize_positions(false);
